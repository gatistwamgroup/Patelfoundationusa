...
class App {
  constructor(
    ...
    // Set initial scroll to the middle of the tripled items for immediate seamless loop
    const totalWidth = this.medias[0].widthTotal / 3; // One set's width
  ...
  createMedias(items, bend = 1, textColor, borderRadius, font) {
    ...
    // Triple the items to ensure enough padding for infinite loop in both directions
  ...
  onCheck() {
    // Disabled snapping to allow smooth continuous scrolling without jumping
  }
  onResize() {
    this.screen = {
  ...
  update() {
    if (!this.isDown) {
      this.scroll.target += 0.15; // Auto-scroll speed
    }
      ...
      ([entry]) => {
        if (entry.isIntersecting) {
          if (!appRef.current) {
            ...
            appRef.current.update(); // Resume loop if already created
          }
        } else {
          if (appRef.current) {
            window.cancelAnimationFrame(appRef.current.raf); // Pause loop
          }
        }
      },
      { threshold: 0.1 }
